...
class SocketClusterService
{
        ...
        while (!$this->sent && $attempts < 2) {
            ...
            try {
                ...
                if (!$this->handshakeSent) {
                    Log::info("SocketCluster: Sending handshake...");
                    $this->handshake(rand());
                    if (!$this->handshakeSent) {
                        Log::error("SocketCluster: Handshake failed (Flag is false)");
                        throw new \Exception('Handshake failed');
                    }
                    Log::info("SocketCluster: Handshake successful.");
                }

                $cid     = rand();
                $message = new SocketClusterMessage($channel, $data, $cid);
                
                Log::info("SocketCluster: Sending message [CID: $cid] to channel [$channel]. Payload: " . (string)$message);

                $this->client->send($message);
                $this->response = $this->client->receive();
                
                if (empty($this->response)) {
                    throw new \Exception("Empty response from SocketCluster (Connection closed?)");
                }

                $jsonResponse = json_decode($this->response, true);
                if (json_last_error() === JSON_ERROR_NONE && isset($jsonResponse['error'])) {
                     $errorCode = $jsonResponse['error']['code'] ?? 'unknown';
                     $errorMessage = $jsonResponse['error']['message'] ?? 'Unknown Error';
                     
                     if ($errorCode == 4009) {
                         $this->handshakeSent = false; // Force re-handshake
                         throw new \Exception("SocketCluster Protocol Error 4009: $errorMessage - Re-handshake required.");
                     }
                     
                     Log::error("SocketCluster Error Response: " . $this->response);
                }

                Log::info("SocketCluster: Message sent. Response: " . $this->response);
                $this->sent     = true;
            } catch (\Throwable $e) {
                $this->error = $e->getMessage();
                Log::error("SocketCluster Send Error (Attempt $attempts): " . $e->getMessage());
            }
        }
    ...
    public function handshake($cid)
    {
        ...
        try {
            ...
            Log::error("SocketCluster Handshake Error (Connection): " . $e->getMessage());
        } catch (\WebSocket\TimeoutException $e) {
            $this->handshakeError = $e->getMessage();
            Log::error("SocketCluster Handshake Error (Timeout): " . $e->getMessage());
        } catch (\Throwable $e) {
            $this->handshakeError = $e->getMessage();
            Log::error("SocketCluster Handshake Error (General): " . $e->getMessage());
        }
