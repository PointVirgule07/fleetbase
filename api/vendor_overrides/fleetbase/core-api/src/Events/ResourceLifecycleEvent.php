...
class ResourceLifecycleEvent implements ShouldBroadcast
{
    ...
    public function __construct(Model $model, $eventName = null, $version = 1)
    {
        ...
        $this->companySession      = session("company", $model->company_uuid);
    ...
    public function broadcastOn(): array|Channel
    {
        ...
        Log::info("ResourceLifecycleEvent broadcastOn: " . $this->broadcastAs(), [
            "channels" => collect($channels)->map(fn($c) => $c instanceof Channel ? $c->__toString() : $c)->toArray(),
            "company_uuid" => $model->company_uuid ?? "null",
            "payload_data" => $this->data
        ]);
    ...
    {
        // Prioritize model's company_uuid over session to prevent stale session data in queue workers
        $companySession = $model->company_uuid ?? session("company");

        // Sync session for other methods
        if ($companySession && session("company") !== $companySession) {
            session(["company" => $companySession]);
            // Clear potentially stale API credential from previous job context
            session()->forget("api_credential");
        }
    ...
    public function getEventData(): array
    {
        ...
        // Ensure UUID and Company UUID are present for Console/Internal usage
        if (is_array($resourceData)) {
            if (!isset($resourceData["uuid"]) && isset($model->uuid)) {
                $resourceData["uuid"] = $model->uuid;
            }
            if (!isset($resourceData["company_uuid"]) && isset($model->company_uuid)) {
                $resourceData["company_uuid"] = $model->company_uuid;
            }
        }
