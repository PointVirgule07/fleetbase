...
class ProcessStripeWebhook implements ShouldQueue
{
    ...
    protected function handleCustomerCreated($stripeCustomer)
    {
        ...
        if ($email) {
            ...
            $companyUuid = env('STRIPE_TARGET_COMPANY_UUID');
            
            if (!$companyUuid) {
                Log::error('STRIPE_TARGET_COMPANY_UUID is not set in environment variables.');
                return;
            }
    ...
    protected function handleCheckoutSessionCompleted($session)
    {
        ...
        $contact = null;
        $companyUuid = null;
        $targetCompanyUuid = env('STRIPE_TARGET_COMPANY_UUID');

        if (!$targetCompanyUuid) {
            Log::error('STRIPE_TARGET_COMPANY_UUID is not set in environment variables.');
            // Fallback or exit? Let's try to proceed but it will likely fail later if no company found.
        }
